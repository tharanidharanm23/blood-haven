...
function DashboardPage() {
  ...
  const setApproval = async (phoneOrEmail: string, approved: boolean) => {
    try {
      await adminSetUserApproval({ data: { adminEmail, phoneOrEmail, approved } });
      setUsers((prev) => prev.map((u) => (u.phone === phoneOrEmail || u.email === phoneOrEmail ? { ...u, approved } : u)));
  ...
  const verifyHospital = async (phoneOrEmail: string, verified: boolean) => {
    try {
      await adminVerifyHospital({ data: { adminEmail, phoneOrEmail, verified } });
      setUsers((prev) => prev.map((u) => (u.phone === phoneOrEmail || u.email === phoneOrEmail ? { ...u, verified } : u)));
  ...
  const removeUser = async (phoneOrEmail: string) => {
    try {
      await adminRemoveUser({ data: { adminEmail, phoneOrEmail } });
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="font-mono text-[10px] tracking-widest uppercase text-muted-foreground">Admin View</div>
      </header>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="hud-panel p-6 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={monthlyRequests}>
              ...
              <XAxis dataKey="month" tick={{ fontSize: 11, fontFamily: "var(--font-mono)" }} stroke="var(--color-muted-foreground)" />
              <YAxis tick={{ fontSize: 11, fontFamily: "var(--font-mono)" }} stroke="var(--color-muted-foreground)" />
              <Tooltip
                contentStyle={{
        ...
        <div className="hud-panel p-6">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={bloodDistribution} dataKey="value" nameKey="name" innerRadius={50} outerRadius={90} stroke="var(--color-surface)" strokeWidth={2}>
                {bloodDistribution.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
              </Pie>
              <Tooltip
                contentStyle={{
                  background: "var(--color-surface)",
      ...
      <div className="hud-panel">
        <div className="p-6 border-b border-border flex justify-between items-center">
          ...
          <span className="font-mono text-[10px] tracking-widest uppercase text-muted-foreground">Requests</span>
        </div>
            ...
            <tbody>
              {requests.map((r) => (
                ...
                >
                  ...
                  <td className="p-3"><BloodTag group={r.bloodGroup} /></td>
                  <td className="p-3 font-mono tabular-nums">{r.units}</td>
                  <td className="p-3"><UrgencyBadge level={r.urgency} /></td>
                  <td className="p-3 font-mono text-xs">{r.status}</td>
                  <td className="p-3 text-right flex items-center justify-end gap-2">
                    <span className="font-mono text-xs text-muted-foreground tabular-nums">{r.postedMinutesAgo}m</span>
                    <button
                      type="button"
                      ...
                      <div className="inline-flex items-center gap-2">
                        <button
                          ...
                          className={`px-2 py-1 border font-mono text-[10px] tracking-widest uppercase font-bold transition-colors ${
                            approved ? "border-border hover:bg-muted" : "border-primary/30 bg-primary/10 text-primary hover:bg-primary/15"
                          }`}
                          disabled={!handle}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                ...
                {donors.map((d) => (
                  <tr key={d.id} className="border-t border-border">
                    <td className="p-3">
                      ...
                      <div className="font-mono text-[11px] text-muted-foreground">{d.phone ?? d.email ?? "-"}</div>
                    </td>
                    <td className="p-3">
                      <BloodTag group={d.bloodGroup} />
                    </td>
                    <td className="p-3">{d.city}</td>
                    <td className="p-3 text-right font-mono tabular-nums">{d.lifetimeDonations ?? 0}</td>
                  </tr>
                ))}
                {donors.length === 0 && (
        ...
        <div className="hud-panel">
          ...
          <div className="divide-y divide-border">
            {hospitalUsers.map((h) => (
              <div key={h.phone ?? h.email ?? h.name} className="p-5 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="font-bold">{h.name}</div>
                  <div className="text-xs text-muted-foreground mt-1">
                    {(h.address ?? "-")} · {(h.district ?? "-")}
                  </div>
                  <div className="font-mono text-xs text-muted-foreground mt-1">{h.phone ?? h.email ?? "-"}</div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span
                    className={`font-mono text-[10px] tracking-widest uppercase px-2 py-1 border ${
                      h.verified ? "text-success border-success/30 bg-success/10" : "text-muted-foreground border-border"
                    }`}
                  >
                  ...
                  <td className="p-3 text-right">
                    <button
                      ...
                      onClick={() => {
                        ...
                        toast.success(nextStatus === "resolved" ? "Report resolved" : "Report reopened");
                      }}
                      className="px-2 py-1 border border-border hover:bg-muted font-mono text-[10px] tracking-widest uppercase font-bold"
                    >
