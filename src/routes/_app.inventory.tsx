      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {total.toLocaleString()} units across {items.length} types · {low} low-stock alert
            {low === 1 ? "" : "s"}
          </p>
        </div>
        <button
            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 hidden md:table-cell font-mono text-muted-foreground tabular-nums">
                    {item.capacity}
                  </td>
                  <td className="p-4 hidden md:table-cell font-mono text-muted-foreground">
                    {item.expiry}
                  </td>
                  <td className="p-4 text-right">
                    {low ? (
                      ...
                      <span className="font-mono text-[10px] tracking-widest uppercase text-success">
                        ● Nominal
                      </span>
                    )}
                  </td>
...
function AddModal({
  ...
  onClose: () => void;
  onAdd: (g: BloodGroup, u: number, exp: string) => Promise<void>;
}) {
    ...
    >
      ...
      >
        ...
        <div className="p-5 border-t border-border flex justify-end gap-2">
          ...
          <button
            onClick={async () => {
              await onAdd(group, units, expiry);
              onClose();
            }}
