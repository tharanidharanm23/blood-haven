...
const LEVELS = ["Local", "District"] as const;
      ...
      {loading ? (
        ...
        <div className="hud-panel divide-y divide-border">
          {results.map((d) => (
            <div
              key={d.id}
              className="p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:bg-muted/40 transition-colors"
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`font-mono text-[10px] tracking-widest uppercase font-bold px-2 py-1 border ${d.available ? "text-success border-success/30 bg-success/10" : "text-muted-foreground border-border"}`}
                >
  ...
  return (
    <div className="hud-panel p-12 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        Try expanding your search or clearing filters.
      </p>
    </div>
  );
}
